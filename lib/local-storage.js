...
Storage.prototype.add_package = function(name, branch, info, callback) {
  var storage = this.storage(name, branch)
...
Storage.prototype.update_versions = function(name, newdata, branch, callback) {
  var self = this
  self._read_create_package(name, function(err, data) {
    ...
    if (change) {
      ...
      self._write_package(name, branch, data, function(err) {
        callback(err, data)
      })
    } else {
      callback(null, data)
...
Storage.prototype.add_version = function(name, version, metadata, branch, tag, callback) {
  var self = this
  self.update_package(name, branch, function updater(data, cb) {
...
Storage.prototype.merge_tags = function(name, branch, tags, callback) {
  var self = this
...
Storage.prototype.add_tarball = function(name, filename, branch) {
  ...
  var storage = self.storage(name, branch)
  if (!storage) {
    process.nextTick(function() {
...
Storage.prototype.get_tarball = function(name, filename, branch, callback) {
  assert(Utils.validate_name(filename))
  var self = this
...
// 6. callback(err?)
//
Storage.prototype.update_package = function(name, branch, updateFn, _callback) {
  var self = this
  var storage = self.storage(name, branch)
  ...
  storage.lock_and_read_json(info_file, function(err, fd, json) {
    ...
    updateFn(json, function(err) {
      ...
      self._write_package(name, branch, json, callback)
    })
  })
}
...
Storage.prototype._write_package = function(name, branch, json, callback) {
...
Storage.prototype.storage = function(package, branch) {
  var path = this.config.get_package_spec(package).storage
  if (path == null) path = this.config.storage + "/" + branch
