...
//
Storage.prototype.add_package = function(name, metadata, branch, callback) {
  var self = this
  ...
  function check_package_local(cb) {
    self.local.get_package(name, {}, branch, function(err, results) {
  ...
  function check_package_remote(cb) {
    self._sync_package_with_uplinks(name, null, {}, branch, function(err, results, err_results) {
      // something weird
      if (err && err.status !== 404) return cb(err)
  ...
  function publish_package(cb) {
    self.local.add_package(name, branch, metadata, callback)
  }
}
...
//
Storage.prototype.add_version = function(name, version, metadata, branch, tag, callback) {
  return this.local.add_version(name, version, metadata, branch, tag, callback)
}
...
// Used storages: local (write)
//
Storage.prototype.merge_tags = function(name, branch, tag_hash, callback) {
  return this.local.merge_tags(name, branch, tag_hash, callback)
}
...
// Used storages: local (write)
//
Storage.prototype.change_package = function(name, metadata, revision, branch, callback) {
  return this.local.change_package(name, metadata, revision, branch, callback)
}
...
// Used storages: local (write)
//
Storage.prototype.add_tarball = function(name, filename, branch) {
  return this.local.add_tarball(name, filename, branch)
}
...
//
Storage.prototype.get_tarball = function(name, filename, branch) {
  ...
  // trying local first
  var rstream = self.local.get_tarball(name, filename, branch)
  var is_open = false
  rstream.on('error', function(err) {
    ...
    self.local.get_package(name, {}, branch, function(err, info) {
      if (!err && info._distfiles && info._distfiles[filename] != null) {
        ...
        self._sync_package_with_uplinks(name, info, {}, branch, function(err, info) {
...
//
Storage.prototype.get_package = function(name, options, branch, callback) {
  if (typeof(options) === 'function') callback = options, options = {}

  var self = this
  self.local.get_package(name, options, branch, function(err, data) {
    if (err && (!err.status || err.status >= 500)) {
      // report internal errors right away
      return callback(err)
    }

    self._sync_package_with_uplinks(name, data, options, branch, function(err, result, uplink_errors) {
      if (err) return callback(err)
...
Storage.prototype._sync_package_with_uplinks = function(name, pkginfo, options, branch, callback) {
  var self = this
  ...
  async.map(uplinks, function(up, cb) {
    ...
    up.get_package(name, branch, _options, function(err, up_res, etag) {
    ...
    self.local.update_versions(name, pkginfo, branch, function(err, pkginfo) {
      if (err) return callback(err)
