...
module.exports = function(config, auth, storage) {
  ...
  // for "npm whoami"
  app.get('/:branch/whoami', function(req, res, next) {
  ...
  // TODO: anonymous user?
  app.get('/:branch/:package/:version?', can('access'), function(req, res, next) {
    var branch = req.params.branch;
    var name = req.params.package.replace(/(@.*\/)/g, "");
    storage.get_package(name, { req: req }, branch, function(err, info) {
  ...
  app.get('/:branch/:package/-/:filename', can('access'), function(req, res, next) {
    var branch = req.params.branch;
    var stream = storage.get_tarball(req.params.package, req.params.filename, branch)
  ...
  // searching packages
  app.get('/:branch/-/all/:anything?', function(req, res, next) {
  ...
  app.post('/:branch/_session', Cookies.express(), function(req, res, next) {
  ...
  app.get('/:branch/-/user/:org_couchdb_user', function(req, res, next) {
    res.status(200)
    next({
  ...
  app.put('/:branch/-/user/:org_couchdb_user/:_rev?/:revision?', function(req, res, next) {
  ...
  // tagging a package
  app.put('/:branch/:package/:tag',
      can('publish'), media('application/json'), tag_package_version)

  app.post('/:branch/-/package/:package/dist-tags/:tag',
      can('publish'), media('application/json'), tag_package_version)

  app.put('/:branch/-/package/:package/dist-tags/:tag',
      can('publish'), media('application/json'), tag_package_version)

  app.delete('/:branch/-/package/:package/dist-tags/:tag', can('publish'), function (req, res, next) {
    var tags = {}
  ...
  app.get('/:branch/-/package/:package/dist-tags', can('access'), function(req, res, next) {
    var branch = req.params.branch;
    storage.get_package(req.params.package, { req: req }, branch, function(err, info) {
      if (err) return next(err)
  ...
  app.post('/:branch/-/package/:package/dist-tags',
  ...
  app.put('/:branch/-/package/:package/dist-tags',
      can('publish'), media('application/json'), expect_json,
      function(req, res, next) {
  ...
  app.delete('/:branch/-/package/:package/dist-tags',
  ...
  app.put('/:branch/:package/:_rev?/:revision?', can('publish'), media('application/json'), expect_json, function(req, res, next) {
    ...
    if (req.params._rev) {
      storage.change_package(name, metadata, req.params.revision, branch, function(err) {
        after_change(err, 'package changed')
      })
    } else {
      storage.add_package(name, metadata, branch, function(err) {
    ...
    function after_change(err, ok_message) {
      ...
      create_tarball(Path.basename(t1), metadata._attachments[t1], branch, function(err) {
        if (err) return next(err)

        var t2 = Object.keys(metadata.versions)[0]
        metadata.versions[t2].readme = metadata.readme != null ? String(metadata.readme) : ''
        create_version(t2, metadata.versions[t2], branch, function(err) {
          if (err) return next(err)

          add_tags(metadata['dist-tags'], branch, function(err) {
            if (err) return next(err)
    ...
    function create_tarball(filename, data, branch, cb) {
      var stream = storage.add_tarball(name, filename, branch)
      stream.on('error', function(err) {
        cb(err)
      })
    ...
    function create_version(version, data, branch, cb) {
      storage.add_version(name, version, data, branch, null, cb)
    }

    function add_tags(tags, branch, cb) {
      storage.merge_tags(name, branch, tags, cb)
    }
  })

  // unpublishing an entire package
  app.delete('/:branch/:package/-rev/*', can('publish'), function(req, res, next) {
  ...
  // removing a tarball
  app.delete('/:branch/:package/-/:filename/-rev/:revision', can('publish'), function(req, res, next) {
  ...
  app.put('/:branch/:package/-/:filename/*', can('publish'), media('application/octet-stream'), function(req, res, next) {
    var name = req.params.package
  ...
  // adding a version
  app.put('/:branch/:package/:version/-tag/:tag', can('publish'), media('application/json'), expect_json, function(req, res, next) {
    var name = req.params.package
    var version = req.params.version
